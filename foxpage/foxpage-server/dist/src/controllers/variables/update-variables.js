...
exports.UpdateVariableDetail = void 0;
...
let UpdateVariableDetail = class UpdateVariableDetail extends base_controller_1.BaseController {
    constructor() {
        super();
    }
    /**
    ...
    async index(ctx, params) {
        ...
        if (!(0, tools_1.checkName)(params.name)) {
            return Response.warning(app_config_1.i18n.variable.invalidVariableName, 2081501);
        }
        try {
            ctx.logAttr = Object.assign(ctx.logAttr, { type: constant_1.TYPE.VARIABLE });
            const hasAuth = await this.service.auth.file(params.id, { ctx });
            if (!hasAuth) {
                return Response.accessDeny(app_config_1.i18n.system.accessDeny, 4081501);
            }
            ...
            const result = await this.service.file.info.updateFileDetail(params, {
                ctx,
                actionType: [constant_1.LOG.UPDATE, constant_1.TYPE.VARIABLE].join('_'),
            });
            if (result.code === 1) {
                return Response.warning(app_config_1.i18n.variable.invalidVariableId, 2081502);
            }
            if (result.code === 2) {
                return Response.warning(app_config_1.i18n.variable.variableNameExist, 2081503);
            }
            this.service.content.info.updateContentItem(contentId, { title: contentName }, { ctx, actionType: [constant_1.LOG.UPDATE, constant_1.TYPE.VARIABLE].join('_') });
            if (versionStatus === constant_1.VERSION.STATUS_BASE) {
                this.service.version.info.updateVersionItem(versionId, { content: params.content }, { ctx, actionType: [constant_1.LOG.UPDATE, constant_1.TYPE.VARIABLE].join('_') });
            }
            else {
                // Add new version
                const version = this.service.version.number.getVersionFromNumber(++versionNumber);
                this.service.version.info.create({ contentId, version, versionNumber, content: params.content }, { ctx, fileId: params.id, actionType: [constant_1.LOG.CREATE, constant_1.TYPE.VARIABLE].join('_') });
            }
            ...
            return Response.error(err, app_config_1.i18n.variable.updateVariableFailed, 3081501);
        }
    }
};
__decorate([
    ...
    (0, routing_controllers_openapi_1.OpenAPI)({
        summary: app_config_1.i18n.sw.updateVariableDetail,
        description: '',
        tags: ['Variable'],
...
], UpdateVariableDetail.prototype, "index", null);
UpdateVariableDetail = __decorate([
    (0, routing_controllers_1.JsonController)('variables'),
    __metadata("design:paramtypes", [])
], UpdateVariableDetail);
exports.UpdateVariableDetail = UpdateVariableDetail;
