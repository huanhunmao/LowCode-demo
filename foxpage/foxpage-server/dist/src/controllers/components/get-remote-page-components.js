...
let GetRemoteComponent = class GetRemoteComponent extends base_controller_1.BaseController {
    ...
    async index(params) {
        try {
            ...
            let resourceData = await this.service.resource.getResourceGroupLatestVersion(params.groupId, { packageName: params.groupName || groupInfo.name, proxy: app_config_1.config.proxy || '' });
            if (params.name) {
                resourceData = lodash_1.default.filter(resourceData, resource => resource.name.indexOf(params.name) !== -1);
            }
            const packageList = lodash_1.default.chunk(resourceData, pageSize.size)[pageSize.page - 1] || [];
            const componentNames = lodash_1.default.map(packageList, 'name');
            const componentInfos = await this.service.component.getComponentInfoByNames(params.applicationId, componentNames);
            const componentNameObject = lodash_1.default.keyBy(componentInfos, 'name');
            const componentIds = lodash_1.default.map(componentInfos, 'id');
            ...
            mapResourceList.forEach((res) => {
                ...
                const componentId = ((_b = componentNameObject[res.name]) === null || _b === void 0 ? void 0 : _b.id) || '';
                const componentMaxVersion = ((_c = maxComponentVersion[componentId]) === null || _c === void 0 ? void 0 : _c.versionNumber) || 0;
...
__decorate([
    (0, routing_controllers_1.Get)('/remote-searchs')
    // @Get('/remote-search')
    ,
    (0, routing_controllers_openapi_1.OpenAPI)({
        summary: app_config_1.i18n.sw.getRemotePageComponents,
        description: '',
        tags: ['Component'],
        operationId: 'get-remote-page-component-list',
    }),
    (0, routing_controllers_openapi_1.ResponseSchema)(component_validate_types_1.RemotePagePackageRes),
    __param(0, (0, routing_controllers_1.QueryParams)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [component_validate_types_1.RemotePagePackageReq]),
