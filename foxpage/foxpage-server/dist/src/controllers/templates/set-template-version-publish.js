...
let SetPageVersionPublishStatus = class SetPageVersionPublishStatus extends base_controller_1.BaseController {
    ...
    async index(ctx, params) {
        try {
            ...
            if (!hasAuth) {
                return Response.accessDeny(app_config_1.i18n.system.accessDeny, 4071401);
            }
            // Set publishing status
            const result = await this.service.version.live.setVersionPublishStatus(params, {
                ctx,
                liveRelation: true,
                actionType: [constant_1.LOG.PUBLISH, constant_1.TYPE.TEMPLATE].join('_'),
            });
            if (result.code === 1) {
                return Response.warning(app_config_1.i18n.template.templateVersionHasPublished, 2071401);
            }
            else if (result.code === 2) {
                return Response.warning(app_config_1.i18n.template.invalidRelations + ':' + Object.keys(result.data).join(','), 2071402);
            }
            ...
            return Response.error(err, app_config_1.i18n.template.setTemplatePublishStatusFailed, 3071401);
        }
    }
};
__decorate([
    ...
    (0, routing_controllers_openapi_1.OpenAPI)({
        summary: app_config_1.i18n.sw.setTemplateVersionPublishStatus,
        description: '',
        tags: ['Template'],
