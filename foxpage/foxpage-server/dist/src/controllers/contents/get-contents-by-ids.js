...
let GetContentByIds = class GetContentByIds extends base_controller_1.BaseController {
    constructor() {
        super();
    }
    /**
    ...
    async index(ctx, params) {
        try {
            ...
            contentList = lodash_1.default.filter(contentList, { deleted: false });
            // Check whether the content belongs to the specified application
            const fileList = await this.service.file.info.getDetailByIds(lodash_1.default.map(contentList, 'fileId'));
            const appFileIds = [];
            ...
            contentList = lodash_1.default.filter(contentList, (content) => !content.deleted && appFileIds.indexOf(content.fileId) !== -1);
            return Response.success(contentList, 1160201);
        }
        catch (err) {
            return Response.error(err, app_config_1.i18n.content.getContentDetailsFailed, 3160301);
        }
    }
};
__decorate([
    ...
    (0, routing_controllers_openapi_1.OpenAPI)({
        summary: app_config_1.i18n.sw.getContentDetails,
        description: '',
        tags: ['Content'],
    ...
    __metadata("design:paramtypes", [Object, content_validate_types_1.ContentDetailsReq]),
    __metadata("design:returntype", Promise)
], GetContentByIds.prototype, "index", null);
GetContentByIds = __decorate([
